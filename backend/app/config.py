...
class Settings(BaseSettings):
    ...
    # Latest Opus (bumped off the 4-6 pin per Chad 2026-07-11). Drives RC's
    # server-side enrichment: prose (listener/societal), ether tagger, album
    # synthesis, identity guard, leit sweep. Prod also sets AGENT_MODEL in .env.
    ...
    # deleted, so calibrate_song_async always calls LEC's /api/score over HTTP.
    # A LEC failure now returns an explicit needs-human-review result rather than
    # re-scoring locally (there is no local scorer). lec_base_url default points
    # at a local LEC; prod points at the internal container (http://lec:8012) /
