...
**bright_green (Ascended):** Actively elevates. Addresses something beyond the self — community, sovereignty, healing, philosophy, the sacred. The parts of human experience that commercial culture has defunded.
...
**green (Elevated):** Processes honestly. Meets the listener at a real emotional place and helps them move through it with dignity.
...
**yellow (Decent):** Moves the listener laterally. Light entertainment that neither serves nor harms. Fills time. Not the problem — but not the solution either. "Decent" is deliberate — this tier is not "balanced" or "healthy." It's adequate. A shrug. Music can do more than this, and the compass exists to show that.
...
- Love + "we make each other better, we grow together" → **elevated** (green). The love is a vehicle for growth.
- Love + "I love you, you love me, we're happy" → **decent** (yellow). It's fine. It's just filling time. Surface-level romance.
...
- Love + "our connection heals us and extends outward to our community" → **ascended** (bright_green). The love transcends the couple.
...
**Contaminated** is NOT a separate tier — it is a modifier that can ONLY attach to **bright_green, green, or yellow** songs.
...
- A green song with real nostalgia contaminated by drug references
- A bright_green song with genuine cultural pride but ego woven through the expression
- A green song with honest emotional processing wrapped in objectification
- A yellow song about family with casual glorification of violence
...
2. A party song is NOT automatically yellow. If it's pure fun with no harmful payload, it's yellow. If it celebrates substance abuse or objectification, it's orange or red.
3. A song about struggle is NOT automatically green. If it processes the struggle with dignity, it's green. If it wallows or glamorizes suffering, it's orange.
...
{
    "rubric_color": "bright_green|green|yellow|orange|red",
...
- **Ascended** (bright_green): +75 to +100
...
def build_few_shot_examples(db: Session) -> str:
    ...
    """
    tiers = ["bright_green", "green", "yellow", "orange", "red"]
    examples = []
