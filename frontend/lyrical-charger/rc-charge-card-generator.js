...
(function () {
  ...
  var SQUARE_H = 1080;   // default card height (1:1 -- public song-page share cards)
  var POST_H = 1350;     // 4:5 portrait height -- broadcaster feed cards (Instagram/X)

  // Resolve the card height from opts.height, falling back to square. Width is
  // always SIZE; only the vertical extent changes (1:1 default vs 4:5 for posts).
  function cardHeight(opts) {
    var h = opts && opts.height;
    return (typeof h === 'number' && h > 0) ? h : SQUARE_H;
  }
  ...
  function drawChrome(ctx, hex, glowY, H) {
    ...
    var bx = 30, bw = SIZE - 60, bh = H - 60, blw = 7;
    ctx.save();
    ctx.beginPath();
