/* === Daily Charge Panel (capsule) ===
   Extracted VERBATIM from js/app.js so the homepage and the chart pages share
   ONE implementation (no drift). Mount:
     DailyChargePanel.mount(containerEl, { loadSeries, loadAnomalies });
   loadSeries(): Promise<[{date, compass_degree, charge_level}]> (trailing series).
   loadAnomalies is optional -- omit on charts that carry no anomalies. Styling is
   the global .trajectory and .traj rules already in css/main.css. */
(function () {
  'use strict';
...
function announce(msg) {
  ...
  // homepage these reach app.js's live compass; the capsule uses safe, self-
  // contained versions so it drives whatever dial is present and no-ops the
  // charge bar / header reframe on pages that don't have them. (Compass is a
  // top-level const, so guard with typeof; Charge may be absent entirely.)
  ...
  function setCompassMode() { /* host-specific compass header reframe; no-op here */ }
  ...
  function createPanel(container, opts) {
    var _loadSeries = opts.loadSeries;
  ...
  function applyTmDrawerState() {
    container.querySelectorAll('.traj-tm-drawer').forEach((drawer) => {
      drawer.classList.toggle('is-open', tmDrawerOpen);
      if (tmDrawerOpen) drawer.removeAttribute('inert');
      else drawer.setAttribute('inert', '');
    });
    container.querySelectorAll('.traj-tm-toggle').forEach((toggle) => {
  ...
  const CAL_MONTHS_FULL = ['January','February','March','April','May','June','July','August','September','October','November','December'];
    ...
    // Scale toggle: the charge-axis labels flip fit <-> full +/-100 (app.js parity).
    ...
    return { load: loadDailyChart, container: container };
  }

  window.DailyChargePanel = {
    mount: function (container, opts) {
      if (!container) return null;
      var p = createPanel(container, opts || {});
      p.load();
      return p;
    }
  };
